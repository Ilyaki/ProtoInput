...
std::vector<unsigned int> MessageList::indexTable{};
std::vector<MessageListItem> MessageList::messages
{
{0	,	"WM_NULL" },
{1	,	"WM_CREATE" },
{2	,	"WM_DESTROY" },
...
void MessageList::GenerateIndexTable()
{
	...
	for (const auto& m : messages)
	{
		...
		for (int j = indexTable.size(); j < m.messageID; j++)
...
bool MessageList::IsBlocked(unsigned int message)
{
	auto ptr = GetBlocked(message);
	return ptr == nullptr ? false : *ptr;
}

bool* MessageList::GetBlocked(unsigned message)
{
	if (static bool hasGenerated = false; !hasGenerated)
	{
		hasGenerated = true;
		GenerateIndexTable();
	}
	...
	return &messages[indexTable[message]].blocked;
}

void MessageList::ShowUI()
{
	...
	ImGui::BeginChild("blocked", ImVec2(0, 0), true);

	for(auto& msg : messages)
	{
		if ((!onlyShowBlocked || msg.blocked)
			&& msg.name.find(search) != std::string::npos
			&& ImGui::Selectable(msg.name.c_str(), msg.blocked))
